import React from 'react'

// component
import Poster from '../component/Posters/PosterComponent'
import PlaysFilter from '../component/PlaysFilters/PlaysFiltersComponent'

const Plays = () => {
    return (
        <>
           <div className="container mx-auto px-4">
           
               <div className="w-full lg:flex lg:flex-row-reverse">
                   <div className="lg:w-3/4 ">
                       <h2 className="text-2xl font-bold mb-4">Plays in Belgaum</h2>    
                        <div className="flex flex-wrap  ">
                            <div className="w-1/2 md:w-1/3 my-5 lg:w-1/4">
                            <Poster src="https://in.bmscdn.com/discovery-catalog/events/tr:w-400,h-600,bg-CCCCCC:w-400.0,h-660.0,cm-pad_resize,bg-000000,fo-top:ote-U2F0LCAzMSBKdWw%3D,ots-29,otc-FFFFFF,oy-612,ox-24/et00312831-rzdltgpbpx-portrait.jpg"
                            title="Admission - 31st July (Saturday)"
                            subtitle="Comedy, Drama | English | 12yrs + | 24hrs"/>
                            </div>
                            <div className="w-1/2 my-5 md:w-1/3 lg:w-1/4">
                            <Poster src="https://in.bmscdn.com/discovery-catalog/events/tr:w-400,h-600,bg-CCCCCC:w-400.0,h-660.0,cm-pad_resize,bg-000000,fo-top:ote-U2F0LCAzMSBKdWw%3D,ots-29,otc-FFFFFF,oy-612,ox-24/et00312831-rzdltgpbpx-portrait.jpg"
                            title="Admission - 31st July (Saturday)"
                            subtitle="Comedy, Drama | English | 12yrs + | 24hrs"/>
                            </div>
                            <div className="w-1/2 my-5 md:w-1/3 lg:w-1/4">
                            <Poster src="https://in.bmscdn.com/discovery-catalog/events/tr:w-400,h-600,bg-CCCCCC:w-400.0,h-660.0,cm-pad_resize,bg-000000,fo-top:ote-U2F0LCAzMSBKdWw%3D,ots-29,otc-FFFFFF,oy-612,ox-24/et00312831-rzdltgpbpx-portrait.jpg"
                            title="Admission - 31st July (Saturday)"
                            subtitle="Comedy, Drama | English | 12yrs + | 24hrs"/>
                            </div>
                            <div className="w-1/2 my-5 md:w-1/3 lg:w-1/4">
                            <Poster src="https://in.bmscdn.com/discovery-catalog/events/tr:w-400,h-600,bg-CCCCCC:w-400.0,h-660.0,cm-pad_resize,bg-000000,fo-top:ote-U2F0LCAzMSBKdWw%3D,ots-29,otc-FFFFFF,oy-612,ox-24/et00312831-rzdltgpbpx-portrait.jpg"
                            title="Admission - 31st July (Saturday)"
                            subtitle="Comedy, Drama | English | 12yrs + | 24hrs"/>
                            </div>
                         </div> 
                   </div>
               
                    <div className="lg:w-3/12">
                            <h2 className="text-2xl font-bold mb-4">Filters</h2>
                            <div>
                                <PlaysFilter title="Date"
                                tags={["Today","Tomorrow","This weekend"]}/>
                                <PlaysFilter title="Language"
                                tags={["Telgue","Tamil","English"]}/>
                                <PlaysFilter title="Categories"
                                tags={["Theater"]}/>
                                <PlaysFilter title="More Filters"
                                tags={["Online Streaming","Kids allowed"]}/>
                                <PlaysFilter title="Price"
                                tags={["Free","0-500","501-2000","Above 2000"]}/>
                            </div>
                    </div>
               </div>
           </div>
        </>
    )
}

export default Plays
